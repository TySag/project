...
load_icode_read(int fd, void *buf, size_t len, off_t offset) {
    ...
    if ((ret = sysfile_seek(fd, offset, LSEEK_SET)) != 0) {
        kprintf("sysfile_seek failed\n");
        return ret;
    }
    if ((ret = sysfile_read(fd, buf, len)) != len) {
        kprintf("sysfile_read failed\n");
        return (ret < 0) ? ret : -1;
    }
    return 0;
}
...
load_icode(int fd, int argc, char **kargv) {
    ...
    struct mm_struct *mm;
    if ((mm = mm_create()) == NULL) {
        kprintf("mm_create failed\n");
        goto bad_mm;
    }
    if (setup_pgdir(mm) != 0) {
        kprintf("set pgdir failed\n");
        goto bad_pgdir_cleanup_mm;
    }
    ...
    if ((ret = load_icode_read(fd, &___elfhdr__, sizeof(struct __elfhdr), 0)) != 0) {
        kprintf("load_icode_read failed\n");
    ...
    for (phnum = 0; phnum < elf->e_phnum; phnum ++) {
      ...
      if ((ret = mm_map(mm, ph->p_va, ph->p_memsz, vm_flags, NULL)) != 0) {
        kprintf("p_va : 0x%x, p_memsz : 0x%x\n", ph->p_va, ph->p_memsz);
        kprintf("1 mm_map failed : %d\n", ret);
        goto bad_cleanup_mmap;
      }
...
do_execve(const char *name, int argc, const char **argv) {
    ...
    //YX DEBUG load_icode
    //int tempi;
    //for(tempi=0; tempi<argc;tempi++){
    //   kprintf("%s\n", kargv[tempi]);
    //}
    //sh 
    //END 
   if ((ret = load_icode(fd, argc, kargv)) != 0) {
        kprintf("load_icode failed\n");
        goto execve_exit;
    }
